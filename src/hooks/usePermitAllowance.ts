...
export function usePermitAllowance(token?: Token, owner?: string, spender?: string) {
  const contract = useContract<Permit2>((token?.chainId == 5701) ? PERMIT2_ADDRESS_ZKSYS : PERMIT2_ADDRESS, PERMIT2_ABI)
...
export function useUpdatePermitAllowance(
  ...
  return useCallback(async () => {
    try {
      ...
      const { domain, types, values } = AllowanceTransfer.getPermitData(permit, (token.chainId == 5701) ? PERMIT2_ADDRESS_ZKSYS : PERMIT2_ADDRESS, chainId)
